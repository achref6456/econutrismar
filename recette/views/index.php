...
<html lang="fr">
  ...
  <body>
    ...
    <?php
      $lanIp = '192.168.1.11';
      $qrUrl = 'http://' . $lanIp . '/EcoNutri/views/index.php';
      $qrEnc = urlencode($qrUrl);
    ?>
    <style>
      #qrFloatBtn{position:fixed;bottom:2rem;left:2rem;width:54px;height:54px;border-radius:50%;background:linear-gradient(135deg,#2d6a1f,#4a9e30);color:#fff;border:none;font-size:1.5rem;cursor:pointer;box-shadow:0 6px 20px rgba(45,106,31,.45);z-index:9999;display:flex;align-items:center;justify-content:center;transition:transform .2s;}
      #qrFloatBtn:hover{transform:scale(1.12);}
      #qrModal{display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:10000;align-items:center;justify-content:center;backdrop-filter:blur(4px);}
      #qrModal.open{display:flex;}
      #qrModalBox{background:#fff;border-radius:24px;padding:2rem;width:min(380px,90vw);box-shadow:0 24px 60px rgba(0,0,0,.3);text-align:center;position:relative;}
      #qrCloseBtn{position:absolute;top:.8rem;right:.8rem;background:#f0f0f0;border:none;width:32px;height:32px;border-radius:50%;cursor:pointer;font-size:1rem;line-height:1;display:flex;align-items:center;justify-content:center;}
    </style>

    <button id="qrFloatBtn" title="QR Code – Partager EcoNutri">📱</button>

    <div id="qrModal">
      <div id="qrModalBox">
        <button id="qrCloseBtn">✕</button>
        <div style="font-size:2rem;margin-bottom:.4rem;">📱</div>
        <h3 style="font-family:'Playfair Display',serif;font-size:1.2rem;color:#2d6a1f;margin-bottom:.3rem;">Scanner pour accéder</h3>
        <p style="font-size:.82rem;color:#666;margin-bottom:1.2rem;">Scannez ce QR code avec votre téléphone pour ouvrir EcoNutri.</p>
        <img id="qrImg"
             src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&color=2d6a1f&bgcolor=ffffff&data=<?= $qrEnc ?>"
             alt="QR Code EcoNutri"
             style="border-radius:12px;box-shadow:0 4px 16px rgba(45,106,31,.15);width:200px;height:200px;display:block;margin:0 auto 1rem;">
        <div style="background:#e8f5e1;border-radius:10px;padding:.6rem 1rem;font-size:.75rem;color:#2d6a1f;font-weight:600;word-break:break-all;margin-bottom:1rem;"><?= htmlspecialchars($qrUrl) ?></div>
        <div style="display:flex;gap:.6rem;justify-content:center;flex-wrap:wrap;">
          <button id="qrCopyBtn" style="background:#e8f5e1;color:#2d6a1f;border:1.5px solid #d9eed0;padding:.45rem 1rem;border-radius:8px;font-size:.82rem;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;">📋 Copier</button>
          <button id="qrDlBtn" style="background:#2d6a1f;color:#fff;border:none;padding:.45rem 1rem;border-radius:8px;font-size:.82rem;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;">⬇️ Télécharger</button>
        </div>
      </div>
    </div>

    <script>
      const _qrUrl = '<?= addslashes($qrUrl) ?>';
      ...
      document.getElementById('qrDlBtn').addEventListener('click', function() {
        fetch(document.getElementById('qrImg').src)
          .then(r => r.blob())
          .then(b => {
            const a = document.createElement('a');
            a.download = 'EcoNutri-QR.png';
